...
一部电梯限载 8 个人、限重 600 公斤。设电梯里有 $x$ 个大人、$y$ 个小孩，那么 $x + y \le 8$ 和 $75x + 40y \le 600$ 这两条规矩没有唯一的答案，它们允许的是一大群数对。把不等式画出来，就是在图上把这一群数对涂成阴影：先不管不等号，把它当方程画出边界线，再试一个好算的点，比如 $(0, 0)$，看看规矩在哪一侧成立，就把那一侧涂上。如果线上的点也算数（$\le$ 或 $\ge$），边界就画实线；不算（$<$ 或 $>$），就画虚线。几个不等式放在一起，允许的区域（也就是"可行域"）就是所有阴影重叠的地方，而它的几个角上藏着最划算的方案，所以从工厂排产到航空公司卖座位，每一个规划问题都从这张图开始。
