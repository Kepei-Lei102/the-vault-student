...
拿一块边长为 $x$ 的正方形瓷砖，在它的下边贴上一条宽为 $b$ 的长条，面积就是 $x^2 + bx$。现在把这条长条沿长边剪成两半，一半贴到正方形的右边，一半贴到下边。这时图形几乎是一个边长为 $x + \frac{b}{2}$ 的更大的正方形，只在右下角缺一个边长为 $\frac{b}{2}$ 的小方块。缺的这个角就是全部的窍门：$x^2 + bx$ 等于 $(x + \frac{b}{2})^2$ 减去那个小角 $(\frac{b}{2})^2$。为什么要费这个劲？因为正方形是可以开平方的。像 $x^2 + 6x = 7$ 这样的方程，写成 $(x + 3)^2 = 16$，就有 $x + 3 = \pm 4$，不用任何公式就把 $x$ 求出来了。求根公式无非就是把这套几何操作用字母一般化地做一遍。
