...
一条笔直高速公路上的两条车道，不管你开多远，永远不会碰到一起。原因不在于它们一开始隔得远，而在于它们朝着同一个方向：每往前走一公里，两条车道爬升或者转向的量都完全一样。在坐标图上，"每向前一步爬升多少"就是斜率，所以两条直线平行，恰好就是它们的斜率相等，比如 $y = 2x + 3$ 和 $y = 2x - 7$。式子末尾的常数只说明这条线从哪里出发；斜率说明它往哪里去，而平行说的全是"往哪里去"。像 $x = 1$ 和 $x = 5$ 这样的竖直线，斜率不存在，没法比较，但它们平行的道理是一样的：两条都笔直朝上。
