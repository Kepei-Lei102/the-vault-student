...
每小时给培养皿里的细菌称一次重，把称出的数画出来：曲线越爬越陡，用尺子怎么也看不出它是每 40 分钟翻一倍，还是每 50 分钟翻一倍。换成画数量的对数试试。如果数量每小时都乘以同一个倍数，对数就每小时增加同样的量，曲线就变成了一条直线，斜率正是这个量。这就是线性化：换一换画的东西，让弯曲的关系变成直线，因为直线是唯一一种拿尺子就能把它的两个参数——斜率和截距——读出来的形状。做法永远一样：先把模型改写成"一个量 = 常数 × 另一个量 + 常数"，再让这两个量充当纵横坐标。幂函数要对两条坐标轴都取对数，这时斜率正好就是原来的指数。这个办法附带两条提醒：截距现在是一个对数，得取反对数把它还原回去，才能找回原来的常数；而数据落成直线，只是支持这个模型，并不能证明它。
