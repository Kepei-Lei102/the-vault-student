...
路边有块牌子写着"12%"。意思是沿着路每走 100 米，路面就升高 12 米：升高的量除以走过的量。这个数就是斜率，一条直路上处处都是这个数，这正是"直"的意思；陡度一直在变的路，就成了一条弯曲的路，也就是曲线。在图上，一条直线的斜率就是它上面任意两点之间 $y$ 的变化除以 $x$ 的变化，因为线是直的，选哪两个点都无所谓。从左往右上坡是正的，下坡是负的，平路是零，而一堵竖直的墙根本没有斜率，因为走过的量是零，零不能做除数。你遇到的每一种"率"，路程-时间图上的速度、每公斤的价钱、每挣一元要交的税，都是藏起来的斜率。
