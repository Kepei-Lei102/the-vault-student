...
分数相加是你很早就会的事：$\tfrac12 + \tfrac13$ 通分以后得到 $\tfrac56$。部分分式就是把这一步倒过来做。给你一个复杂的分式，比如 $\dfrac{3x+1}{(x-1)(x+3)}$，你要问的是：它是哪几个简单分式加出来的？分母的每个因式，各对应其中一个。答案是 $\dfrac{1}{x-1} + \dfrac{2}{x+3}$。别人加起来的东西，为什么要费劲拆开？因为拆开的每一块都好办，合在一起就不好办：每一块积分出来是一个对数，可以展开成级数，或者一眼就能看出它的性质，而合起来的那个分式把这些全都藏了起来。每一块长什么样，由分母的因式决定；上面的数字靠"配"出来，要么比较系数，要么挑几个让某个因式变成零的 $x$ 值代进去。动手之前先看一个条件：分子的次数必须比分母低，否则要先用除法把多项式部分分出来，再拆剩下的真分式。
...
通分是把简单分式拼成复杂分式；部分分式是把复杂分式拆成简单分式。学了之后才能用对数和反正切积分有理函数，也是 [[Binomial Series|二项级数]]展开的入口。
