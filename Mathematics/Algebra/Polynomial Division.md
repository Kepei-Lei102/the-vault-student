...
十七颗糖分给五个孩子：每人三颗，剩下两颗，这和写成 $17 = 5 \times 3 + 2$ 是一回事。多项式除法就是这道算式里多了个 $x$。用 $x^3 + 2x$ 除以 $x^2 - 1$，问的是被除数里装得下几个除数、还剩多少，答案也是同一个样子：$x^3 + 2x = (x^2 - 1)\cdot x + 3x$，一个商，一个余。唯一的规矩也和分糖一样：余数必须比除数小，而对多项式来说，"小"指的是次数低，所以剩下的部分次数一比除数低，就可以停了。竖式的做法和做数字除法时完全一样，从最高次项开始，一项一项往下做。值得学它，是因为这一步别的工具替不了：余式定理只管 $x - a$ 这种除数，而一个假分式（分子次数不低于分母、"头重脚轻"的那种），得先把多项式那部分除出来，才能拆成部分分式。
