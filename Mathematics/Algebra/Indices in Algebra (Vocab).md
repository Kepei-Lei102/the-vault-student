...
一块正方形地毯，边长是 $x$ 米，面积就是 $x^2$ 平方米。如果把边长都扩大到原来的3倍，新地毯的一排能放下3块原来的地毯，一共又有3排，所以面积变成9倍，不是3倍。写成 $(3x)^2$ 时，平方管的是整个 $3x$，而不只是 $x$；所以结果是 $9x^2$。字母并没有改变乘法的道理，它只是替那个还没给出具体数值的边长占了个位置。
