...
试一下：随便挑一个奇数，平方，再减 1。3 得 8，5 得 24，7 得 48，11 得 120。每一个答案都是 8 的倍数。你可以再试一百个，个个都成立，可你还是不知道第一百零一个成不成立，因为一百个例子，也保证不了你没试过的那一个。代数证明是把它们一次全试完。奇数总能写成 2n + 1，n 是某个整数，于是它的平方减 1 就是 (2n + 1)² − 1 = 4n² + 4n = 4n(n + 1)。n 和 n + 1 是两个相邻的整数，其中必有一个是偶数，所以 n(n + 1) 是 2 的倍数，4n(n + 1) 就是 8 的倍数。干活的是那个字母 n：它一上场，2n + 1 就把所有奇数同时代表了，所以这段论证把没人试过的那个情况也一并罩住了。证明就是干这个用的，这也是为什么例子再多，也永远不是证明。
