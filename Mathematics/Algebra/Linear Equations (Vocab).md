...
一架天平两端平衡：一边放着一个封好的盒子和三个 1 千克的砝码，另一边放着十一个 1 千克的砝码。盒子有多重？两边各拿掉三个砝码，天平还是平衡的：只剩下盒子，对着八个砝码。这就是一元一次方程，$x + 3 = 11$，也是每一个一元一次方程的解法：对两边做同样的事，直到未知数单独留在一边。两边同时减去同一个数、加上同一个数、除以同一个数，天平永远不会倾斜。"一次"的意思是未知数只是它自己出现，从不平方，也从不和自己相乘，所以这样一步一步总能走到头，答案是唯一的一个数，把它代回去一验就知道对不对。
