...
把一个球抛出去，问它什么时候正好在两米高。实打实的答案有两个：上去的时候一次，下来的时候一次，这就是为什么二次方程有两个解：高度随时间画出来是一条只拐一次弯的曲线，一条水平线可以穿过它两次，碰它一下，或者根本碰不到。每一个二次方程 $ax^2 + bx + c = 0$ 问的都是这个问题。因式分解是在两个交点的横坐标恰好是一眼就能看出的整数时，把它们找出来；配方法是把方程改写成以拐点为中心的形式，于是两个答案就落在拐点两边同样远的地方；求根公式则是把配方法一劳永逸地做完，而判别式 $b^2 - 4ac$ 让你还没开始算就知道，那条水平线是穿过曲线两次、只碰一下，还是根本碰不到。三种方法，一幅画。
