...
出租车起步价 10 元，之后每公里 2 元。把里程当横轴、车费当纵轴画出来，点全落在一条直线上：它从纵轴上的 10 出发，因为那是车还没动就要付的钱；每往右走一公里就升高 2 元，因为那是单价。这就是 $y = mx + c$，其中 $m = 2$，$c = 10$：斜率是每公里的价钱，截距是起步价，图像之所以是直的，是因为单价从头到尾不变。每一个线性图像讲的都是这样一个故事：一个起始的量，加上一个固定的速率。读图就是认出哪个数是哪个：图像和纵轴相交的地方是"还没开始就有的"，它爬升的陡度是"每一单位多多少"。
