...
抛四次硬币，恰好两次正面朝上有多少种情况？把四次抛掷写成四个格子，挑出哪两个是正面：正正反反、正反正反、正反反正、反正正反、反正反正、反反正正，一共六种。现在来展开 $(a+b)^4$。四个括号相乘，每一项都是从每个括号里选 $a$ 或者选 $b$，所以恰好含两个 $a$ 的项，数出来正是刚才那六种选法，于是出现了 $6a^2b^2$。展开式里的每一个系数都是一次"选法的计数"，这就是为什么 1、4、6、4、1 这一行恰好也是杨辉三角的一行：每个数都表示"从四个里挑若干个"，一共有多少种挑法。这个定理不是一条要背的公式，它说的是：展开括号和数选法，是同一件事。
