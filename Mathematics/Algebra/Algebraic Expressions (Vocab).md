...
菜谱上写着“每人两个鸡蛋”。它没有告诉你该买几个鸡蛋，因为它不知道会来多少人；它告诉你的是，知道了人数以后该怎么算。把人数记作 n，菜谱就成了 2n，这就是一个代数表达式：一个留着空位、等数字填进来的算式。它不是一个问题，也还没有答案，这正是它没有等号的原因。填进 n = 6，它给出 12；填进 20，它给出 40；表达式本身是那条规则，而不是其中任何一个结果。等你写下 2n = 12，你就是在问一件事了：用掉了一打鸡蛋，来的是几个人？这才是方程。对表达式做的所有操作——合并同类项、去括号、因式分解——都是在改写这条规则，却不改变它对任何一个 n 给出的结果；只有方程才谈得上“解”。
