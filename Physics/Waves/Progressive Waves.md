...
体育场里的人浪绕着看台飞快地跑，可没有一个观众离开过座位。每个人只是站起来再坐下，比旁边的人晚一点点。这个"晚一点"就是行波的全部秘密：沿途传过去的是动作的样子，不是人。抖一下晾衣绳的一端，一个"包"沿着绳子跑过去，绳子本身哪儿也没去，每一小段绳子只是比前一段晚一点起伏。波的那条公式也藏在这同一个画面里：你的手决定每秒发出几个"包"，绳子有多紧、有多重决定每个"包"跑多快，两者合起来就定下了一个"包"有多长。于是波速等于每秒经过的个数乘以每个的长度，也就是波速 = 频率 × 波长。

### 术语对照 (Terms)

行波 progressive wave · 波源 source · 介质 medium · 振幅 amplitude · 周期 period · 频率 frequency · 波长 wavelength · 波速 wave speed · 相位 phase · 横波 transverse wave · 纵波 longitudinal wave · 强度 intensity
