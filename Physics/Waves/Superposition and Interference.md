...
往平静的池塘里隔开一点扔两颗小石子。每颗石子各自荡开一圈圈水波，两组水波互相穿过，在重叠的地方出现一幅固定的图样：有几条线上，水起伏得足有原来的两倍高，夹在它们之间的另几条线上，水几乎不动，尽管两组波正从那里经过。道理很简单：两列波相遇的地方，水面的位移就是两列波各自单独造成的位移加在一起。波峰和波峰同时到，就叠高；波峰和波谷同时到，就互相抵消。某个位置到底是哪一种，只取决于它到两颗石子的距离差了多少：差的正好是整数个波长，两列波步调一致地到达；再多出半个波长，就正好步调相反。降噪耳机是故意利用"抵消"这一半：麦克风听到噪声，耳机立刻播放一个同样的、只是上下颠倒过来的波，让波峰总是碰上波谷，传到耳朵里的就几乎是一片安静。要让图样稳定不动，两个波源必须始终保持步调一致；两盏各自独立的灯做不到这一点，所以两盏灯只会更亮，不会出现条纹。

### 术语对照 (Terms)

叠加原理 principle of superposition · 干涉 interference · 相长干涉 constructive interference · 相消干涉 destructive interference · 路程差 path difference · 相位差 phase difference · 相干 coherent · 杨氏双缝 Young's double slit · 条纹间距 fringe spacing · 拍 beats
