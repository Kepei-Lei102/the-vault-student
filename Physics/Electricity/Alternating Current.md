...
墙上插座里的电不是朝一个方向流的，而是来回晃：在中国，电流每秒换向一百次（每秒五十个周期，50 Hz），电压在 +311 V 和 −311 V 之间摆动。那标签上为什么写 220 V？因为每个周期里有一半时间电压远远低于峰值，而电热壶在乎的是发热，发热看的是电压的平方。把电压的平方在一个周期里取平均，正好是峰值平方的一半；再开方，就是峰值除以根号二：$311/1.414 = 220\ \text{V}$。所以 220 V 指的是：换成多大的直流电压，才能把同一壶水烧得一样快。这个值叫有效值（也叫均方根值，英文 r.m.s. value）。你见过的每一个市电电压（中国 220 V、日本 110 V、美国 120 V）都是有效值，背后都藏着一个根号二倍大的峰值。
