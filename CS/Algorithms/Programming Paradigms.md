...
店里想算出所有不到六元的商品一共值多少钱，你可以交代得很细：逐件看价格，合适的就加进总数；也可以把商品写成对象，让商品对象自己判断是否符合条件；还可以直接向数据库说明“把价格不到六元的那些记录加起来”，把具体怎么查交给它。要算的东西没变，变的是你怎样组织这个任务、哪些步骤由自己安排、哪些交给现成的机制。编程范式就是这样一种组织思路。说“我要什么”并没有让步骤消失，只是有人已经把执行这些步骤的工具做好了。
