...
**自动化系统**是让传感器、微处理器和执行器协同工作，从而**无需人的介入**就能完成动作的系统。传感器不断把对物理世界的测量数字化后送给微处理器；微处理器把读数与预设值比较后做出判断；执行器把判断变成物理动作——开阀、转动电机、加热——而这个动作改变的，恰好就是传感器正在测量的那个量，于是这个环又转一圈。恒温器是最小的一个例子：测温、比较、开关加热器、再测温。**机器人学**是计算机科学的一个分支，研究机器人的设计、制造和运行。**机器人**有三个特征：一个**机械结构**（框架、机械臂、底盘、旋翼），**电气部件**（传感器、微处理器、执行器），以及**可编程**（它按可以修改的指令行动）；大多数机器人还能移动或移动自身的一部分。判断一件东西是不是机器人，最容易漏看的是第一层——智能音箱有芯片有程序，却没有框架、没有任何能动的部件，所以不是机器人；最容易被加码的是第三层——"可编程"只是说它执行程序，并不要求它会思考、会学习或自己拿主意，由外科医生远程操控的手术机器人仍然是机器人。用一个 Python 模拟把一个最简单的机器人（前方 10 厘米内有物体就转向）放进一个房间："永远右转"这条死规则让它在一条 12.5 米的闭环上永远打转，只覆盖了 14.5% 的地面；把规则换成随机角度反弹，走 300 米就覆盖约 90%。同一套传感器、微处理器、执行器，只有程序不同——这就是"可编程"这三个字的分量。
