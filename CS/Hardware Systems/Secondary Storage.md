...
The 2027 guide explicitly names internal and external secondary storage in **A1.1.7**: SSD, HDD, eMMC, optical drives, flash drives, memory cards and NAS, with usage scenarios. Use the comparisons here to justify a device choice. This is narrower than claiming all A1.1 coverage: CPU/GPU, primary memory, pipelining, compression and cloud service models have separate outcomes. A1.3.2 names OS memory management; it does not explicitly prescribe every buffering or virtual-memory mechanism discussed here.
