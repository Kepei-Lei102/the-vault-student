...
想象两个车站的售票窗口，彼此之间的网络突然断了。一边刚卖掉最后一个座位，另一边的屏幕却还显示“有票”。这时，确认不了最新情况的窗口，是先暂停或拒绝售票，还是照着手头的记录继续答复？前者让人等，后者可能给出过时的信息。把数据复制到更多电脑上，确实能让更多人同时办事，可副本之间一旦联系不上，就不能保证既有求必应，又始终给出同一个最新答案。这是分布式数据难伺候的地方：多放几份不难，难的是联系中断时，还要决定该信哪一份。这道限制不只属于 NoSQL；分布式关系型数据库也一样要面对。
