...
班长改了一个同学的电话号码，却发现班级名单改好了，社团名单和春游名单里还是旧号码。问题不在于谁粗心，而在于同一个“当前号码”被抄了好几遍。换个办法：学生表里只存一份当前号码，其他名单记这个同学的学号；春游负责人要联系他时，就凭学号去查。这样改一处，大家下次查到的就是同一份新信息。关系型数据库最值得先抓住的直觉，就是把描述不同对象的事实分开放，再靠稳定的编号接起来。至于曾经用过的号码，那是另一类历史事实；需要保留时，就单独记录。
...
| Foreign key | 外键 | 本表中的字段（或字段组合），引用本表或另一张表中能唯一标识记录的键——**联系就靠它** |
| Candidate / secondary key | 候选键 / 辅助键 | 能唯一标识一行的最小字段组合 / 用来快速查找的非唯一字段 |
