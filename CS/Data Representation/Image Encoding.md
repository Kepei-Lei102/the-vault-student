...
把手机里的照片一直放大，画面会碎成一个个小色块；把 PDF 里的文字一直放大，笔画却永远锐利。差别在于存下来的是什么。照片存的是一格一格的采样：相机在几百万个点上各量了一次颜色，把这些数字记了下来，所以放大时并没有新的东西可看，只是把同样的方格画大了。PDF 里的文字存的却是画法：从这里到那里画一条曲线，填成黑色。放大就是按新的尺寸把画法再执行一遍，每次都是重新画出来的。照片只能存采样，因为现实世界不附带画法；而 logo、字体、示意图是人设计出来的，画法本来就有，存画法更划算。
