...
游戏设置里，音乐、字幕、震动各有一个开关。程序可以把这些“开或关”挤进同一个整数里，每一位管一个开关。你关掉音乐，并不想连字幕和震动也一起关掉，所以程序不能把整个数随便改掉：它要找到管音乐的那一位，只改那里。位运算就是这样把一个数拆开看，直接拨动里面的小开关；掩码则像一张只在目标位置开了孔的纸，帮你选中要动的那一位。
